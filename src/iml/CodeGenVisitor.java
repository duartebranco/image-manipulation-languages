...
public class CodeGenVisitor extends imlBaseVisitor<String> {
    ...
    private String ensureKernelIsNumpy(String kernelVar) {
        if (!convertedKernels.contains(kernelVar)) {
            ...
            return (
                kernelVar + " = np.array(" + kernelVar + ", dtype=np.uint8)\n"
            );
        }
    ...
    @Override
    public String visitStatement(imlParser.StatementContext ctx) {
        String res = null;
    ...
    public String visitVariableDeclaration(
        imlParser.VariableDeclarationContext ctx
    ) {
    ...
    @Override
    public String visitAssignment(imlParser.AssignmentContext ctx) {
    ...
    @Override
    public String visitIfStatement(imlParser.IfStatementContext ctx) {
    ...
    @Override
    public String visitUntilStatement(imlParser.UntilStatementContext ctx) {
    ...
    private void visitWithIndent(
        imlParser.StatementContext ctx,
        String indent
    ) {
        visit(ctx);
    }
    ...
    @Override
    public String visitOutputStatement(imlParser.OutputStatementContext ctx) {
        sb.append("print(").append(visit(ctx.expression())).append(")\n");
        return null;
    }

    @Override
    public String visitDrawStatement(imlParser.DrawStatementContext ctx) {
    ...
    public String visitStoreStatement(imlParser.StoreStatementContext ctx) {
        ...
        sb
            .append("        _pil_images_for_gif[0].save(")
            .append(pythonStringLiteralForSave)
            .append(
                ", save_all=True, append_images=_pil_images_for_gif[1:], duration=100, loop=0)\n"
            );
        sb.append("    else:\n");
        sb
            .append("        print(f\"Warning: Image list '")
            .append(exprToStore.replace("\"", "\\\""))
            .append("' is empty. Cannot save GIF to {'")
            .append(pathContentForFString)
            .append("'}\")\n");
        sb.append("elif isinstance(img_store, np.ndarray):\n");
        sb.append("    if img_store.ndim == 2:\n");
        sb
            .append(
                "        Image.fromarray(np.clip(img_store * 255, 0, 255).astype(np.uint8)).save("
            )
            .append(pythonStringLiteralForSave)
            .append(")\n");
        sb.append("    else:\n");
        sb
            .append(
                "        Image.fromarray(np.clip(img_store * 255, 0, 255).astype(np.uint8), 'RGB').save("
            )
            .append(pythonStringLiteralForSave)
            .append(")\n");
        sb.append("else:\n");
        sb
            .append(
                "    print(f\"Error: Cannot store type {type(img_store)} as image/GIF for expression '"
            )
            .append(exprToStore.replace("\"", "\\\""))
            .append("'.\")\n");
        return null;
    ...
    @Override
    public String visitAppendStatement(imlParser.AppendStatementContext ctx) {
        int before = sb.length();
    ...
    public String visitLoadExpr(imlParser.LoadExprContext ctx) {
        ...
        return (
            "(lambda _p: np.array(Image.open('examples/' + _p).convert('L')) / 255.0 if _p.lower().endswith(('.pgm')) else np.array(Image.open('examples/' + _p).convert('RGB')) / 255.0)(" +
            pathExpr +
            ")"
        );
    }
    ...
    @Override
    public String visitStringConversionExpr(
        imlParser.StringConversionExprContext ctx
    ) {
        String expr = visit(ctx.expression());
        return "str(" + expr + ")";
    }
    ...
    @Override
    public String visitNumberConversionExpr(
        imlParser.NumberConversionExprContext ctx
    ) {
        String expr = visit(ctx.expression());
    ...
    public String visitErodeExpr(imlParser.ErodeExprContext ctx) {
        ...
        sb
            .append("    ")
            .append(tempVar)
            .append(" = np.stack([cv2.erode(")
            .append(image)
            .append("[...,c], ")
            .append(kernel)
            .append(", iterations=1) for c in range(")
            .append(image)
            .append(".shape[2])], axis=-1)\n");
        sb.append("else:\n");
        sb
            .append("    ")
            .append(tempVar)
    ...
    public String visitOpenExpr(imlParser.OpenExprContext ctx) {
        ...
        sb
            .append(tempVar)
            .append(" = cv2.morphologyEx(")
            .append(input)
            .append(", cv2.MORPH_OPEN, ")
            .append(kernel)
            .append(")\n");
        return tempVar;
    }
    ...
    public String visitTopHatExpr(imlParser.TopHatExprContext ctx) {
        ...
        sb.append(
            ...
            " = cv2.morphologyEx((" +
            img +
            " * 255).astype(np.uint8), cv2.MORPH_TOPHAT, " +
            kernel +
    ...
    public String visitBlackHatExpr(imlParser.BlackHatExprContext ctx) {
        ...
        sb.append(
            ...
            " = cv2.morphologyEx((" +
            img +
            " * 255).astype(np.uint8), cv2.MORPH_BLACKHAT, " +
            kernel +
            ").astype(np.float32) / 255.0\n"
        );
        return resultVar;
    ...
    @Override
    public String visitType(imlParser.TypeContext ctx) {
        String res = null;
        return visitChildren(ctx);
        //return res;
    }
    ...
    @Override
    public String visitPixelOperationExpr(
        imlParser.PixelOperationExprContext ctx
    ) {
        String left = visit(ctx.left);
        String right = visit(ctx.right);
        String op = ctx.operator.getText();
    ...
    public String visitScaleExpr(imlParser.ScaleExprContext ctx) {
        ...
        sb
            .append("    ")
            .append(tempVar)
            .append(" = np.array(Image.fromarray(np.clip((")
            .append(left)
            .append(") * 255, 0, 255).astype(np.uint8)).resize((")
            .append(width)
            .append(", ")
            .append(height)
            .append("))) / 255.0\n");
        ...
        sb
            ...
            .append(" = np.array(Image.fromarray(np.clip((")
            .append(left)
            .append(") * 255, 0, 255).astype(np.uint8), mode='RGB').resize((")
            .append(width)
            .append(", ")
            .append(height)
            .append("))) / 255.0\n");
    ...
    public String visitFlipExpr(imlParser.FlipExprContext ctx) {
        ...
        switch (op) {
            ...
            case "+": // both
                sb
                    .append(tempVar)
                    .append(" = np.flipud(np.fliplr(")
                    .append(image)
                    .append("))\n");
                break;
            default:
                throw new IllegalArgumentException(
                    "Unknown flip operator: " + op
                );
        }
        return tempVar;
    }
    ...
    @Override
    public String visitUnaryPixelOperationExpr(
        imlParser.UnaryPixelOperationExprContext ctx
    ) {
    ...
    @Override
    public String visitPixelBoolOperationExpr(
        imlParser.PixelBoolOperationExprContext ctx
    ) {
        String left = visit(ctx.left);
        String right = visit(ctx.right);
        String op = ctx.operator.getText();
        switch (op) {
            ...
            default:
                throw new RuntimeException(
                    "Unknown pixel-wise boolean op: " + op
                );
        }
    }

    @Override
    public String visitUnaryPixelBoolOperationExpr(
        imlParser.UnaryPixelBoolOperationExprContext ctx
    ) {
        String operand = visit(ctx.expression());
        return "~" + operand;
    }
    ...
    @Override
    public String visitTryCatchStatement(
        imlParser.TryCatchStatementContext ctx
    ) {
        sb.append("try:\n");
